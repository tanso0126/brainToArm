...
2026-08-02 실물 사진으로 하부 통풍구 본체가 팔과 같은 `+X` 방향임을
확인했습니다. 이전 모델은 원본 `Alt_Kasa.stl`의 출력판 방향을 그대로 써
본체를 반대 방향에 표시했고, 실물 인터록도 방향성 없는 낮은 원통으로
근사해 `[90, 95, 165, 140, 170, 180]` 자세를 잘못 허용했습니다. 이
자세에서 손목이 본체 전면 상단에 걸려 모터가 손상됐습니다.

현재 모델은 다음 전방 방향성 상자를 사용합니다.
...
MuJoCo 시각 메시도 전방으로 뒤집고 통풍구 덮개 메시를 추가했습니다.
실물 안전 코드와 시뮬레이터가 같은 본체 방향·크기·여유를 사용하며, 위
사고 자세와 그 자세로 향하는 중간 경로는 `base-housing` 충돌로 영구
거부됩니다. 학습 파지 자세는 이 여유를 모두 적용한 뒤에도 약 10mm 이상
남는 후보만 사용합니다.
