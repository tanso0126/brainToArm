...
def fit_floor_mount(samples: Sequence[FloorCalibrationSample],
    ...
    initial = np.asarray(
        initial if initial is not None else [0.015, 0.055, 25.0], dtype=float)
    if initial.shape != (3,) or not np.isfinite(initial).all():
        raise ValueError("initial sonar mount must contain three finite values")
    ...
    bounds = ([-0.12, -0.08, -20.0], [0.20, 0.20, 85.0])
    ...
    if len(inlier_indices) >= 4 and len(inlier_indices) < len(samples):
