...
LIFTED_OPEN = [90, 110, 90, 180, 90, 170]
...
class PhysicalArmSafetyTests(unittest.TestCase):
    ...
    def test_physically_exercised_reference_poses_remain_available(self):
        for pose in (HOME, LIFTED_OPEN):
            with self.subTest(pose=pose):
                self.assertTrue(self.safety.pose_report(pose).safe)
        self.assertTrue(self.safety.transition_report(HOME, LIFTED_OPEN).safe)

    def test_real_table_contact_pose_is_no_longer_called_hover(self):
        for pose in (HOVER, GRASP):
            with self.subTest(pose=pose):
                report = self.safety.pose_report(pose)
                self.assertFalse(report.safe)
                self.assertIn("finger-table", report.explain())
    ...
    def test_server_interlock_prevents_serial_write(self):
        ...
        self.assertEqual(arm.pose, LIFTED_OPEN)
    ...
    def test_table_touch_command_moves_only_the_bounded_table_plane(self):
        ...
        # The historical touch poses were planned to the virtual grasp centre,
        # not the newly modelled physical finger endpoint. Lowering the assumed
        # table plane by 18 mm must not make a 41 mm finger penetration legal.
        with self.assertRaisesRegex(RuntimeError, "finger-table"):
            server.handle({
