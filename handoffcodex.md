...
> 아래 2026-07-27 Codex 추가 기록이 이후 상태를 갱신한다. 원래 내용의
> "물리 grasp 미완"은 고정 위치 시험 뒤 잠시 성공으로 봤으나, 후속 무개입
> 시험에서 slipped-object false positive가 발견되어 다시 미완으로 분류한다.

## 2026-07-27 Codex 추가 — 고정 위치 close/lift 후 자율 재검증 중

- `arm_fk.py`가 오래된 시뮬레이터 관절 변환을 사용해 손끝 높이를 크게
  오판한 것이 "물체 위에서 닫힘"의 직접 원인이었다. 현재 MuJoCo 관절 맵과
  수치 오차 없이 일치하도록 고쳤다.
- 물체 bbox 안쪽의 크기 비례 pinch line을 손가락 접근 기하에 맞춘다.
- hover에서 선택한 `reach=37`을 하강 중 끝까지 잠그고, shoulder만 보상해
  손끝을 35→6mm로 내렸다. 가림이 생겨도 다른 FastSAM blob을 쫓지 않는다.
- 실제 장난감차 시험에서 hover `du=21,dv=21`, 최종 바닥
  `du=10,dv=23`; 닫은 뒤 jaw residual 203px, 들어 올린 뒤 약 196px로
  두 번 모두 `CONTACT`였다. 실제 hold 후 같은 자리에 안전하게 내려놓았다.
- 실제 증거와 재현 명령은 `docs/PHYSICAL_GRASP_VALIDATION.md`에 있다.
- 다물체 선택 및 `reject -> next`도 이 실물 수직-pinch controller에 직접
  연결했다. 다만 목표 2/3의 별도 **실물** 판정은 화면에 도달 가능한 물체가
  두 개 놓인 상태에서 각각 실행해야 하며, 소프트웨어 테스트만으로 완료라
  부르지 않는다.
- 이후 HOME부터 시작한 무개입 시험에서 물체가 들기 중 미끄러졌는데도 기존
  7px contact threshold가 성공으로 판정하는 사례를 발견했다. 성공 기준을
  80mm lift + 최초 obstruction 50% 이상 유지로 강화했으며, 일반 목표 1도
  다시 미완으로 분류한다. 상세 실패 기록은
  `docs/AUTONOMOUS_GRASP_VALIDATION.md`에 있다.
